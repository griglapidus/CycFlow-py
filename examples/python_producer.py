...
class SineProducer(cyclib.BatchRecordProducer):
    ...
    def define_rule(self) -> cyclib.RecRule:
        return cyclib.make_rule([
            ("Sample", cyclib.DataType.Float),
            ("Index",  cyclib.DataType.UInt32),
        ])

    def produce_batch(self, batch: cyclib.WriteBatch) -> int:
        arr = batch.as_numpy()
        n = batch.capacity
